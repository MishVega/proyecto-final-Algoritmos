...
def menu_busquedas():
    while True:
        ...
        print("2. Buscar Mision por Prioridad (Lineal)")
        ...
        if opcion == "1":
            ...
            buscar_lineal_mision_prioridad()
        elif opcion == "3":
...
# =====================================================================
# 5. ÁRBOL BINARIO DE BÚSQUEDA (BST) DE MISIONES
# =====================================================================

arbol_misiones = []
raiz_arbol_misiones = -1
def insertar_mision_arbol(nueva_mision):
    global raiz_arbol_misiones
    
    nodo_nuevo = {
        "codigo": nueva_mision["codigo"],
        "zona": nueva_mision["zona"],
        "tipo_emergencia": nueva_mision["tipo_emergencia"],
        "izq": -1,  # Posición del hijo izquierdo (-1 = vacío)
        "der": -1   # Posición del hijo derecho (-1 = vacío)
    }
    arbol_misiones.append(nodo_nuevo)
    posicion_nuevo = len(arbol_misiones) - 1
    # CASO 1: Si el árbol está vacío, se convierte en la raíz principal
    if raiz_arbol_misiones == -1:
        raiz_arbol_misiones = posicion_nuevo
        print(f"Misión {nueva_mision['codigo']} colocada como la raíz del árbol.")
        return
    # CASO 2: Si ya hay datos, caminamos manualmente comparando los códigos
    posicion_actual = raiz_arbol_misiones
    while True:
        nodo_actual = arbol_misiones[posicion_actual]
        # Si el nuevo código es menor alfabéticamente, se va a la izquierda
        if nueva_mision["codigo"] < nodo_actual["codigo"]:
            if nodo_actual["izq"] == -1:
                nodo_actual["izq"] = posicion_nuevo
                print(f"Misión {nueva_mision['codigo']} insertada a la IZQUIERDA de {nodo_actual['codigo']}.")
                break
            else:
                posicion_actual = nodo_actual["izq"]
        # Si el nuevo código es mayor o igual, se va a la derecha
        else:
            if nodo_actual["der"] == -1:
                nodo_actual["der"] = posicion_nuevo
                print(f"Misión {nueva_mision['codigo']} insertada a la DERECHA de {nodo_actual['codigo']}.")
                break
            else:
                posicion_actual = nodo_actual["der"]
def buscar_mision_arbol():
    print("\n========== BÚSQUEDA EN ÁRBOL (BST) ==========")
    if raiz_arbol_misiones == -1:
        print("El árbol está vacío. No hay misiones que buscar.")
        return
    codigo_buscar = input("Ingrese el código de la misión a buscar en el árbol (ej: M001): ").strip()
    posicion_actual = raiz_arbol_misiones
    encontrado = False
    while posicion_actual != -1:
        nodo_actual = arbol_misiones[posicion_actual]
        print(f"Visitando nodo... (Código: {nodo_actual['codigo']})")
        if nodo_actual["codigo"] == codigo_buscar:
            print("\nEncontrado")
            print(f"-> Datos: Zona: {nodo_actual['zona']} | Emergencia: {nodo_actual['tipo_emergencia']}")
            encontrado = True
            break
        elif codigo_buscar < nodo_actual["codigo"]:
            posicion_actual = nodo_actual["izq"]
        else:
            posicion_actual = nodo_actual["der"]
    if not encontrado:
        print(f"La misión '{codigo_buscar}' no se encuentra en el árbol.")
def recorrido_preorden(posicion):
    """Raíz -> Izquierda -> Derecha"""
    if posicion != -1:
        nodo = arbol_misiones[posicion]
        print(nodo["codigo"], end=" ")        # Muestra la Raíz primero
        recorrido_preorden(nodo["izq"])      # Va a la Izquierda
        recorrido_preorden(nodo["der"])      # Va a la Derecha

def recorrido_inorden(posicion):
    """Izquierda -> Raíz -> Derecha (Los muestra ordenados alfabéticamente)"""
    if posicion != -1:
        nodo = arbol_misiones[posicion]
        recorrido_inorden(nodo["izq"])       # Va a la Izquierda
        print(nodo["codigo"], end=" ")        # Muestra la Raíz al medio
        recorrido_inorden(nodo["der"])       # Va a la Derecha

def recorrido_postorden(posicion):
    """Izquierda -> Derecha -> Raíz"""
    if posicion != -1:
        nodo = arbol_misiones[posicion]
        recorrido_postorden(nodo["izq"])     # Va a la Izquierda
        recorrido_postorden(nodo["der"])     # Va a la Derecha
        print(nodo["codigo"], end=" ")        # Muestra la Raíz al final

def menu_recorridos():
    if raiz_arbol_misiones == -1:
        print("El árbol está vacío. No hay recorridos que mostrar.")
        return
    print("\n--- RECORRIDOS DEL ÁRBOL ---")
    print("Preorden:  ", end="")
    recorrido_preorden(raiz_arbol_misiones)
    print("\nInorden:   ", end="")
    recorrido_inorden(raiz_arbol_misiones)
    print("\nPostorden: ", end="")
    recorrido_postorden(raiz_arbol_misiones)
    print() # Salto de línea final

# --- MENÚ DEL ÁRBOL BST ---
def menu_arbol_misiones():
    while True:
        print("\n==============================")
        print(" ÁRBOL BINARIO DE BÚSQUEDA (BST)")
        print("==============================")
        print("1. Insertar Misión en el Árbol")
        print("2. Buscar Misión en el Árbol")
        print("3. Mostrar Recorridos Obligatorios")
        print("4. Regresar")
        opcion = input("Seleccione una opción: ")
        if opcion == "1":
            print("\n--- Insertando misiones actuales del sistema ---")
            if not misiones:
                print("No hay misiones registradas en el sistema principal. Regístralas primero.")
            else:
                arbol_misiones.clear()
                global raiz_arbol_misiones
                raiz_arbol_misiones = -1
                for m in misiones:
                    insertar_mision_arbol(m)
                print("¡Todas las misiones del sistema han sido indexadas en el árbol!")
        elif opcion == "2":
            buscar_mision_arbol()
        elif opcion == "3":
            menu_recorridos()
...
def menu_principal():
    while True:
        ...
        print("6. Búsqueda de Drones")
        print("7. Gestionar Misiones Pendientes")
        print("8. Arbol Binario")
        print("9. Simulación de Rescate")
        print("10. Salir")
        ...
        elif opcion == "8":menu_arbol_misiones()
        elif opcion == "9":simulacion_rescate()
        elif opcion == "10":
...
# ==================================================
# SIMULACIÓN COMPLETA DE RESCATE
# ==================================================
def simulacion_rescate():
    print("\n==========================================")
    print("     SIMULACIÓN COMPLETA DE RESCATE")
    print("==========================================")
    # -----------------------------
    # PASO 1
    # -----------------------------
    print("\nPASO 1: Registrar la misión")
    if len(misiones) == 0:
        print("No existen misiones registradas.")
        return
    # Se toma la primera misión pendiente
    mision = misiones[0]
    print("Misión seleccionada:")
    print("Código:", mision["codigo"])
    print("Zona:", mision["zona"])
    print("Emergencia:", mision["tipo_emergencia"])
    print("Prioridad:", mision["prioridad"])
    # -----------------------------
    # PASO 2
    # -----------------------------
    print("\nPASO 2: Insertar la misión en la cola FIFO")
    cola_fifo_misiones.append(mision)
    print("La misión fue agregada a la cola.")
    # -----------------------------
    # PASO 3
    # -----------------------------
    print("\nPASO 3: Insertar en el BST")
    insertar_mision_arbol(mision)
    print("Misión insertada en el árbol correctamente.")
    # -----------------------------
    # PASO 4
    # -----------------------------
    print("\nPASO 4: Ordenar las misiones por prioridad")
    bubble_sort_prioridad()
    print("Misiones ordenadas correctamente.")
    # -----------------------------
    # PASO 5
    # -----------------------------
    ...
    dron = None
    for d in drones:
        if d["estado"].strip().lower() == "disponible":
            dron = d
            break
    if dron is None:
        print("No existe ningún dron disponible.")
        return
    print("Dron encontrado:")
    print("Código:", dron["codigo"])
    print("Modelo:", dron["modelo"])
    # -----------------------------
    # PASO 6
    # -----------------------------
    print("\nPASO 6: Verificar ruta mediante BFS")
    existe = bfs("Base", mision["zona"])
    if not existe:
        print("No existe una ruta hacia la zona.")
        return
    # -----------------------------
    # PASO 7
    # -----------------------------
    print("\nPASO 7: Calcular ruta mínima con Dijkstra")
    dijkstra("Base", mision["zona"])
    # -----------------------------
    # PASO 8
    # -----------------------------
    print("\nPASO 8: Asignar dron")
    dron["estado"] = "Ocupado"
    print("Dron", dron["codigo"], "asignado correctamente.")
    # -----------------------------
    # PASO 9
    # -----------------------------
    print("\nPASO 9: Actualizar estados")
    mision["estado"] = "Atendida"
    print("Estado misión:", mision["estado"])
    print("Estado dron:", dron["estado"])
    print("\n==========================================")
